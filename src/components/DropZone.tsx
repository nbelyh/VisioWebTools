...
export const DropZone = (props: {
  ...
  const className = `drop-zone-base ${dragging ? 'drop-zone-dragging' : 'drop-zone-default'}`;
  ...
  return (
    <div className="flex mb-4">
        ...
        <div className={className} onDragOver={onDragOver} onDrop={onDrop} onDragLeave={onDragLeave}>
          {file
            ? <div>
                ...
                <input type="button" value="Reset" onClick={() => setFile()} className="btn-neutral" />
              </div>
            : <div>
              <p>{props.label}</p>
              <button className="btn-neutral" onClick={onUploadButtonClick}>Or click here pick a file...</button>
            </div>
          }
        </div>
      ...
      <div className="md:w-1/6 p-4">
        <a href={`/samples/${props.sampleFileName}`} onDragStart={onDragStart} draggable="true" className="sample-file-link">
